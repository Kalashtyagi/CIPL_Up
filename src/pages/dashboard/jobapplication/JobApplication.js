...
const JobApplication = () => {
  ...
  const [showSerialId, setShowSerialId] = useState("");
  ...
  const getJobApplications = async (selection) => {
    ...
    try {
      ...
      console.log('71  jobapllication data list',response.data.data);
    } catch (error) {
  ...
  const handleStatus = async (e) => {
    ...
    try {
      const request = await axios.post(
        `${process.env.REACT_APP_API_URL}job-applications/statusupdate/${showModalId}`,
        { status: id },
        {
          headers: {
          ...
          {jsonData?.data?.userPermissions?.find((x) =>
            ...
            <button>
              ...
              {showStatusModal === false ? null : row?.id === showModalId ? (
                <div className={`absolute  z-50  p-1 bg-gray-50 md:w-fit border border-gray-200 h-fit  left-0 ${showSerialId>=5?"bottom-10":"top-10"}`}  >
                  <div className=" ">
...
<div className="p-4 col-span-3">
  ...
  />
        ...
        <div className="flex justify-center items-center h-64">
          ...
          <div className="grid grid-cols-1 bg-white border-t-2 my-2 border-gray-900 shadow hello custom-table ">
